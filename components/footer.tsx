export default function Footer() {
  return (
    <footer id="footer" className="mx-auto w-full flex-grow px-4 py-12 z-20">
        <p className="block mb-5 text-center font-semibold">You can also contact me on</p>
        <div className="flex justify-center gap-1">
            <a href="https://www.linkedin.com/in/liz-anguren/" className="social-link inline-flex h-11 w-11 items-center justify-center" aria-label="LinkedIn profile">
                <svg width="35" height="35" viewBox="0 0 20 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M17.619 0H2.38095C1.06667 0 0 1.12 0 2.5V18.5C0 19.88 1.06667 21 2.38095 21H17.619C18.9333 21 20 19.88 20 18.5V2.5C20 1.12 18.9333 0 17.619 0ZM6.19048 8V17.5H3.33333V8H6.19048ZM3.33333 5.235C3.33333 4.535 3.90476 4 4.7619 4C5.61905 4 6.15714 4.535 6.19048 5.235C6.19048 5.935 5.65714 6.5 4.7619 6.5C3.90476 6.5 3.33333 5.935 3.33333 5.235ZM16.6667 17.5H13.8095C13.8095 17.5 13.8095 12.87 13.8095 12.5C13.8095 11.5 13.3333 10.5 12.1429 10.48H12.1048C10.9524 10.48 10.4762 11.51 10.4762 12.5C10.4762 12.955 10.4762 17.5 10.4762 17.5H7.61905V8H10.4762V9.28C10.4762 9.28 11.3952 8 13.2429 8C15.1333 8 16.6667 9.365 16.6667 12.13V17.5Z" fill="currentColor"/>
                </svg>

            </a>
            <a href="https://github.com/LizAntoinette" className="social-link inline-flex h-11 w-11 items-center justify-center" aria-label="GitHub profile">
                <svg width="35" height="35" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M10.5 0C4.70138 0 0 4.77499 0 10.6644C0 15.6616 3.388 19.8429 7.9555 21C7.9065 20.856 7.875 20.689 7.875 20.4819V18.6592C7.44888 18.6592 6.73487 18.6592 6.5555 18.6592C5.83712 18.6592 5.19837 18.3454 4.88862 17.7625C4.54475 17.1146 4.48525 16.1237 3.633 15.5176C3.38013 15.3159 3.57262 15.0857 3.864 15.1168C4.40212 15.2714 4.84837 15.6465 5.26837 16.2028C5.68663 16.76 5.8835 16.8862 6.66487 16.8862C7.04375 16.8862 7.61075 16.864 8.1445 16.7787C8.4315 16.0384 8.92762 15.3567 9.534 15.035C6.0375 14.6698 4.36888 12.903 4.36888 10.5044C4.36888 9.47177 4.802 8.47287 5.53788 7.63127C5.29638 6.7959 4.99275 5.09226 5.63063 4.4435C7.20388 4.4435 8.155 5.47973 8.38338 5.75967C9.16738 5.48684 10.0284 5.3322 10.9331 5.3322C11.8396 5.3322 12.7041 5.48684 13.4899 5.76145C13.7156 5.48328 14.6676 4.4435 16.2444 4.4435C16.8849 5.09314 16.5777 6.80389 16.3336 7.63749C17.0651 8.47732 17.4956 9.47355 17.4956 10.5044C17.4956 12.9013 15.8296 14.6671 12.3384 15.0342C13.2991 15.5434 14 16.9742 14 18.0522V20.4819C14 20.5743 13.9799 20.641 13.9694 20.7201C18.0609 19.2635 21 15.3176 21 10.6644C21 4.77499 16.2986 0 10.5 0Z" fill="currentColor"/>
                </svg>

            </a>
        </div>

       
        <p className="text-muted text-center mt-10 text-[15px]">
                © 2026 - Liz Antoinette Anguren
        </p>
       
    </footer>
  );
}
